...
const scoreOptions = (value) => `<option value=""${value == null || value === "" ? " selected" : ""}>Not specified</option>${Array.from({ length: 11 }, (_, score) => `<option value="${score}"${selected(String(value), String(score))}>${score}</option>`).join("")}`;
const athleteContext = (athleteNotes, exportOptions) => `<section class="panel athlete-context"><h2>Athlete Notes</h2><div class="athlete-context-controls"><label><span>RPE (0-10)</span><select name="rpe">${scoreOptions(athleteNotes.rpe)}</select></label><label><span>Fatigue (0-10, 10 = extremely fatigued)</span><select name="fatigue">${scoreOptions(athleteNotes.fatigue)}</select></label><label><span>Position</span><select name="position"><option value=""${athleteNotes.position ? "" : " selected"}>Not specified</option><option value="seated"${selected(athleteNotes.position, "seated")}>Seated</option><option value="standing"${selected(athleteNotes.position, "standing")}>Standing</option><option value="mixed"${selected(athleteNotes.position, "mixed")}>Mixed</option></select></label></div><label class="athlete-notes"><span>Notes</span><textarea name="athlete-notes" placeholder="Optional workout context">${escape(athleteNotes.notes)}</textarea></label><label class="export-laps"><input name="include-individual-laps" type="checkbox"${checked(exportOptions.includeIndividualLaps)} /> Include individual laps in exports</label></section>`;
...
export const renderApp = (state) => {
  ...
  const { activity, selection, result, lapsResult, settings, ftp, maxHeartRate, chartOptions, athleteNotes, exportOptions } = state;
  const start = activity.metadata.startTime;
