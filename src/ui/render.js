...
const timeline = (activity, selection, chartOptions) => {
  ...
  return `<section class="panel timeline"><div class="section-heading"><div><h2>Selection</h2><p class="quiet">Elevation profile, power, cadence, and heart rate</p></div><span class="selection-label">${escape(selectionLabel)}</span></div>
    <div class="selection-presets"><button class="${active(selection.type === "activity")}" data-action="select-activity" type="button">Whole activity</button><button class="${active(selection.type === "range")}" data-action="select-range" type="button">Custom range</button>${activity.laps.map((lap) => `<button class="${active(selection.type === "lap" && selection.lapIndex === lap.index)}" type="button" data-action="select-lap" data-index="${lap.index}">${escape(lap.title)}</button>`).join("")}</div>
...
const summary = (result, units, ftp) => `<section class="panel"><h2>Summary</h2><dl class="metrics">
  ...
  ${metric("Distance", result.summary.distanceMeters == null ? null : formatMetricDistance(result.summary.distanceMeters, units))}
  ${metric("Average speed", result.summary.averageSpeedMps == null ? null : formatSpeed(result.summary.averageSpeedMps, units))}
  ${metric("Maximum speed", result.summary.maximumSpeedMps == null ? null : formatSpeed(result.summary.maximumSpeedMps, units))}
...
const zones = (result) => !result.zones.length ? "" : `<section class="panel"><h2>Power zones</h2><div class="zones">${result.zones.map((zone) => `<div><strong>${zone.label}</strong><span>${formatDuration(zone.durationSeconds)} · ${formatPercent(zone.percentage)}</span></div>`).join("")}</div></section>`;

const laps = (lapsResult, units) => !lapsResult.length ? "" : `<section class="panel"><h2>Laps</h2><div class="laps">${lapsResult.map((lap) => `<article><h3>${lap.title}</h3><p>${formatDuration(lap.summary.activeDurationSeconds)} · ${formatMetricDistance(lap.summary.distanceMeters, units)}</p><p>NP ${formatMetric(lap.power.normalizedPowerWatts, "W")} · Power ${formatMetric(lap.power.averageWatts, "W")} · HR ${formatMetric(lap.heartRate.averageBpm, "bpm")}</p><button type="button" data-action="select-lap" data-index="${lap.index}">Select lap</button></article>`).join("")}</div></section>`;
...
export const renderApp = (state) => {
  ...
  const { activity, selection, result, lapsResult, settings, ftp, ftpSource, exportOptions, chartOptions } = state;
  const start = activity.metadata.startTime;
  return `<main><header class="app-header"><div><p class="eyebrow">actex</p><h1>${escape(activity.metadata.sport ?? "Cycling")}</h1><p>${formatDate(start)} · ${formatDuration(result.summary.activeDurationSeconds)} · ${formatMetricDistance(result.summary.distanceMeters, settings.units)}</p>${activity.metadata.productName ? `<p class="quiet">${escape(activity.metadata.productName)}</p>` : ""}</div><button class="primary" type="button" data-action="copy-markdown">${state.copyStatus ?? "Copy for AI"}</button></header>
  <section class="ftp-inline"><strong>FTP</strong><label><input name="ftp" type="number" min="1" inputmode="numeric" value="${ftp ?? ""}" aria-label="FTP in watts" /> W</label><span class="quiet">${escape(ftpSource)}</span><button type="button" data-action="set-ftp">Apply</button><button type="button" data-action="save-ftp">Save default</button></section>
  ...
  ${summary(result, settings.units, ftp)}
  ${zones(result)}${laps(lapsResult, settings.units)}${quality(result)}
  ...
  <details class="panel"><summary>Settings</summary><label>Units <select name="units"><option value="metric"${selected(settings.units, "metric")}>Metric</option><option value="imperial"${selected(settings.units, "imperial")}>Imperial</option></select></label><div class="zone-settings">${settings.zones.filter((zone) => zone.id !== "coasting").map((zone) => `<label>${zone.label} starts at % FTP <input name="zone-${zone.id}" type="number" min="0" step="0.1" value="${zone.minimum * 100}" /></label>`).join("")}</div><button type="button" data-action="save-settings">Save settings</button><button type="button" data-action="reset-settings">Reset settings</button></details>
