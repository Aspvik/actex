...
export const buildCompactText = (model) => [
  `${model.activity.sport ?? "Activity"}, ${formatDate(new Date(model.activity.startTimestamp))}`,
  `${formatDuration(model.summary.activeDurationSeconds)}, ${formatMetricDistance(model.summary.distanceMeters)}, ${formatSpeed(model.summary.averageSpeedMps)}`,
